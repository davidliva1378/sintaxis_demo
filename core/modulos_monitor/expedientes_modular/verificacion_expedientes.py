    ...
    async def verificar_async(self):
        try:
            ...
            if page:
                ...
                base_dir = os.path.dirname(os.path.abspath(__file__))
                ruta_config = os.path.normpath(os.path.join(base_dir, "../../../config/config_monitor.json"))

                if not os.path.exists(ruta_config):
                    # fallback: intentar en raíz del proyecto
                    ruta_config_alt = os.path.normpath(os.path.join(base_dir, "../../../config_monitor.json"))
                ...
                if fecha_corte_iso:
                    try:
                        fecha_corte = datetime.strptime(fecha_corte_iso, "%Y-%m-%d").strftime("%d/%m/%Y")
                    except Exception as e:
                        registrar_log(f"⚠️ Error al convertir fecha de corte: {fecha_corte_iso} → {e}")
                        fecha_corte = None
                else:
                    fecha_corte = None
