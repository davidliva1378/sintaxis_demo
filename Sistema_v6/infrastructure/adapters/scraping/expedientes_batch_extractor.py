"""Extractor batch de expedientes del portal PJN.

Módulo para extraer expedientes en lote desde el portal PJN,
con soporte para ordenamiento, filtrado y detección de duplicados.
"""
...
async def extraer_expedientes(
    page: Page,
    carpeta_salida: str = "data/expedientes",
    delay: int = 3000,
    ...
    """Extrae expedientes del portal PJN en modo batch.

    Args:
        page: Página de Playwright ya autenticada
        carpeta_salida: Ruta donde guardar los expedientes
        delay: Delay entre operaciones (ms)
        nombre_archivo: Nombre del archivo JSON a guardar
        detener_en_duplicado: Si True, detiene al encontrar un duplicado
        guardar_json: Si True, guarda los resultados en JSON
        fecha_corte: Fecha en formato DD/MM/YYYY para detener extracción
        tiempo_maximo_segundos: Tiempo máximo de extracción
        orden: Ordenar por: fecha, caratula, oficina, situacion

    Returns:
        Tupla (lista_expedientes, ruta_archivo, estado)
    """
    try:
    ...
    if orden:
        ...
        if valor_orden:
            try:
                await page.select_option(r"#j_idt150\:order_by_form\:camara", value=valor_orden)
