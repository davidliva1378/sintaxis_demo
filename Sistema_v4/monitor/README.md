...
* Se reactivó la comparación automática y manual de expedientes mediante el helper `comparar_expedientes`, notificando cambios desde la bandeja y registrando los informes generados en `datos_extraidos/monitoreo/reportes`. Cuando hay una fecha de corte configurada, la base histórica se filtra automáticamente antes de comparar para evitar falsos positivos masivos y se registran avisos con el detalle del filtrado. Si el helper no está disponible en la instalación, la acción queda deshabilitada y se informa al operador.【F:Sistema_v4/monitor/comparacion_expedientes.py†L1-L181】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L351-L361】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L523-L570】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L796-L813】
...
* `comparacion_expedientes.py` centraliza la resolución de rutas, el manejo de faltantes y la detección del informe generado, filtrando la base histórica con el mismo corte configurado antes de delegar en `comparar_con_base`. De esta forma solo se reportan diferencias reales en entornos con extracción acotada por fecha.【F:Sistema_v4/monitor/comparacion_expedientes.py†L1-L181】
