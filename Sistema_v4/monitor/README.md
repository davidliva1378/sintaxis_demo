...
* La configuración lee `config/config_monitor.json`, soporta modos `automatico`, `laboral` y `no_laboral`, expone un bloque `filtro_expedientes` con las opciones heredadas (`hoy`, `ultimo_dia_habil`, `dias_atras`, combinadas) y añade `comparacion.modo` para alternar entre contraste parcial (filtrado por fecha de corte) o total sin recortes. Todo puede modificarse desde el submenú “🛠️ Modo de trabajo” o editando el JSON según corresponda, y los cambios reinician los temporizadores al vuelo.【F:Sistema_v4/monitor/configuracion_modo.py†L8-L97】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L330-L414】
...
* Se reactivó la comparación automática y manual de expedientes mediante el helper `comparar_expedientes`, notificando cambios desde la bandeja y registrando los informes generados en `datos_extraidos/monitoreo/reportes`. El helper respeta el modo configurado: “parcial” filtra la base con la fecha de corte resuelta y “total” compara contra todos los expedientes históricos, dejando avisos en el log para documentar la estrategia aplicada. Si el helper no está disponible en la instalación, la acción queda deshabilitada y se informa al operador.【F:Sistema_v4/monitor/comparacion_expedientes.py†L1-L222】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L34-L105】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L523-L570】【F:Sistema_v4/monitor/monitor_entradas_expedientes_v4.py†L796-L813】
