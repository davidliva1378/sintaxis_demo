...
NOTIF_DIR = r"C:\\Users\\aleja\\OneDrive\\Escritorio\\Proyecto-Descargas\\Notificaciones"
...
def actualizar_notificaciones_nuevas(page: Page):
    ...
    scroll_contenedor = page.query_selector(SELEC_CONTENEDOR_SCROLL)
    ...
    while intentos_sin_nuevos < max_intentos:
        filas = page.query_selector_all(SELEC_TABLA)
        nuevos_en_scroll = 0

        for fila in filas:
            try:
                num_elem = fila.query_selector(SELEC_EXPEDIENTE_NUMERO)
                car_elem = fila.query_selector(SELEC_EXPEDIENTE_CARATULA)
                celdas = fila.query_selector_all("td")

                if not num_elem or not car_elem or len(celdas) < 3:
                    continue

                numero = limpiar_texto(num_elem.inner_text())
                caratula = limpiar_texto(car_elem.inner_text())
                fecha_str = limpiar_texto(celdas[2].inner_text())
        ...
        page.evaluate("(sel) => document.querySelector(sel).scrollBy(0, 1500);", SELEC_CONTENEDOR_SCROLL)
        time.sleep(1)
